...
public class BackroomsGamerules {
    ...
    // Register Enum game rule, appears on the world creation screen under the
    // backrooms category and command /gamerule respawn
    public static final GameRules.Key<EnumRule<Respawn>> RESPAWN = GameRuleRegistry.register("respawn",
            new CustomGameRuleCategory(new Identifier("backrooms", "backrooms_category"), Text.of("Backrooms")),
            GameRuleFactory.createEnumRule(Respawn.natural, (server, rule) -> {
                onRespawnGameruleChanged(server, rule.get());
            }));

    public static void init() {
        // ENTITY_LOAD is called whenever an entity is loaded (new chunk, natural spawn,
        // player joins world, player respawns)
        // When the entity is a player and RESPAWN is set to natural, reset the
        // SpawnPoint overwrite
    ...
    // when the rule is changed, iterate over every player and set the right spawn
    // or reset the overwrite
    ...
    // sets the spawn according to RESPAWN, only when player is inside a backrooms
    // level
    ...
    private static void setSpawnpointToCurrentLevel(ServerPlayerEntity player) {
        ...
        // uses current y of the player so the respawn point is inside the level and not
        // over or under it
        ...
        boolean up = false;
        // searches for a place inside the level where two blocks are air so the player
        // can spawn
    ...
    private static void setSpawnpointToLevel0(ServerPlayerEntity player) {
        ...
        mutBlockPos = new BlockPos(newX, newY, newZ).mutableCopy();
        boolean up = false;
        // searches for a place inside level 0 where two blocks are air so the player
        // can spawn
