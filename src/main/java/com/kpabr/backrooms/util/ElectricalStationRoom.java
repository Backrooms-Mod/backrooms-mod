...
public class ElectricalStationRoom {
	...
	public ElectricalStationRoom(int floor, int startX, int startZ, long seed) {
		Random random = new Random(seed + BlockPos.asLong(startX, startX, floor));
	...
	public static ElectricalStationRoom hallwayBetween(ElectricalStationRoom room1, ElectricalStationRoom room2,
			Direction direction) {
		...
		if ((direction == Direction.EAST && !room1.eastHallway) || (direction == Direction.WEST && !room2.eastHallway)
				|| (direction == Direction.SOUTH && !room1.southHallway)
				|| (direction == Direction.NORTH && !room2.southHallway)) {
			result.setValues(0, 15, 0, 15);
		}
		return result;
	}
