...
public class ParkingGarageChunkGenerator extends ChunkGenerator {
    public static final Codec<ParkingGarageChunkGenerator> CODEC = RecordCodecBuilder.create((instance) -> instance
            .group(
        ...
        for (int y = floorCount; y >= 0; y--) {
            ...
            for (int x = 3; x >= 0; x--) {
                for (int z = 3; z >= 0; z--) {
                    if ((x & 1) == 1) {
                        // Create a pillar.
                        chunk.setBlockState(new BlockPos(startX + x * 4 + 3, 2 + 8 * y, startZ + z * 4 + 3),
                                BackroomsBlocks.PARKING_CEMENT_PILLAR.getDefaultState(), false);
                        chunk.setBlockState(new BlockPos(startX + x * 4 + 3, 3 + 8 * y, startZ + z * 4 + 3),
                                BackroomsBlocks.PARKING_CEMENT_PILLAR.getDefaultState(), false);
                        chunk.setBlockState(new BlockPos(startX + x * 4 + 3, 4 + 8 * y, startZ + z * 4 + 3),
                                BackroomsBlocks.CEMENT_PILLAR.getDefaultState(), false);
                        chunk.setBlockState(new BlockPos(startX + x * 4 + 3, 5 + 8 * y, startZ + z * 4 + 3),
                                BackroomsBlocks.CEMENT_PILLAR.getDefaultState(), false);
                    }
                    // Generate the carpeting and the ceiling.
                    for (int i = 0; i < 4; i++) {
                        for (int j = 0; j < 4; j++) {
                            chunk.setBlockState(new BlockPos(startX + x * 4 + i, 1 + 8 * y, startZ + z * 4 + j),
                                    BackroomsBlocks.WOOLEN_CARPET.getDefaultState(), false);
                            for (int k = 0; k < 3; k++) {
                                chunk.setBlockState(new BlockPos(startX + x * 4 + i, 6 + 8 * y + k, startZ + z * 4 + j),
                                        BackroomsBlocks.CORK_TILE.getDefaultState(), false);
                            }
                        }
                    }
                    // Place a ceiling light at the correct height.
                    chunk.setBlockState(new BlockPos(startX + x * 4 + 1, 6 + 8 * y, startZ + z * 4 + 1),
    ...
    public void storeStructures(ServerWorld world) {
        ...
        // store("parking_garage_nofill", world, 0, 0); //Makes it so the large nofill
